...
> 当前交付基线：单台 Linux 服务器 8 核 CPU / 32 GB RAM、Docker Compose 的待验证起步形态；在真实设备、目标负载和恢复演练通过前，不以“万台”、RPO 或 RTO 对外承诺。
> 说明：这是统一沟通版，随 `delivery/leadership-reports/` 管理汇报材料维护（2026-08-20 由 docs/design/ 迁入，DEC-20260820-004）；服务、数据、支付退款、协议和实时推送的可执行细节以《[架构基线与服务边界](../../docs/design/充电运营平台架构基线与服务边界-v2.md)》及《[Kafka 事件目录](../../contracts/events/README.md)》为准。本文件的总图只表达公司级业务域和边界，不表示所有 Kafka 订阅、Schema 或存储连线。

## 1. 一句话说明

平台是一个可独立部署的充电运营系统：现行七服务负责租户、场站、设备、充电订单、客户权益、资金、运维、经营分析和国内省市监管；设备运行数据通过统一 MQTT 协议进入，支付、银行和门禁等交易/运营渠道通过 `integration-service` 接入，监管由 `ecosystem-service` 在 W10 达到**内部就绪**。真实目标接入另以 C3-R 生产 Go 门禁验收。
...
国内充电运营平台的公司级图应优先表达“谁服务谁、设备和业务怎样分开、哪些外部机构由谁对接”，而不是把数据库、协议细节和每条 Topic 堆进一张图。华为云的充电桩参考架构将设备/边缘接入与云端业务应用拆开；腾讯云则把设备上报、平台下行控制和转发给业务系统区分为不同方向。[华为云智慧充电桩场景](https://support.huaweicloud.com/intl/zh-cn/productdesc-iothub/iot_04_0044.html) [腾讯云物联网数据流转](https://cloud.tencent.com/document/product/1081/134731)

- 实线表示用户请求、外部 API 或设备协议会话；虚线只表示已经持久化的内部业务事件。
- 图中底部的 Kafka 只表示跨域事件协作；具体 Topic、发布者和消费者组仍以事件目录为准。
- 外部机构与专用服务的直连线表达业务归属；实际 HTTP 回调和对外 API 仍经 `api-gateway` 的统一入口与审计。
- MySQL、Redis、ClickHouse、MinIO、监控和备份属于运行底座，故不在公司级图中重复连线；数据所有权见第 5 节。

```mermaid
flowchart TB
  CLIENT["七端业务入口\nP1 / P2 / M1–M4 / D1"] -->|"HTTPS / WSS"| ACCESS["统一访问边界\nNginx + api-gateway"]

  subgraph BUSINESS["运营与交易域"]
    direction LR
  ...
  subgraph DEVICE_DOMAIN["设备与实时数据域"]
    direction LR
    EMQX["EMQX\nMQTT 会话、认证、路由"]
    DEVICE["device-connectivity-service\n协议校验、控制下发、回执"]
    TELEMETRY["telemetry-service\n实时状态、分钟遥测、分析、大屏"]
    EMQX <--> DEVICE
  end
  ACCESS --> TELEMETRY

  subgraph EXTERNAL["专用外部边界"]
    direction LR
    CHANNEL["支付 / 银行 / 门禁\n开票 / 通知渠道"]
    REGULATOR["中国国内省级监管\n+ 可选省内市级目标"]
    UPSTREAM["独立：合规上游 IOT\n协同：仅本方 IOT"]
    IOT_SYNC["本方 IOT\n仅协同模式的管理同步"]
  end
  CHANNEL <-->|"外部 API / 回调"| INTEGRATION
  REGULATOR <-->|"报送 / 回执 / 对账"| ECOSYSTEM
  UPSTREAM <-->|"统一 MQTT"| EMQX
  IOT_SYNC -->|"HTTP / HTTPS 管理同步"| DEVICE

  KAFKA["Kafka\n已登记的版本化业务事件"]
  PLATFORM -.->|"业务事件"| KAFKA
  FINANCE -.->|"资金事件"| KAFKA
  INTEGRATION -.->|"渠道结果"| KAFKA
  ECOSYSTEM -.->|"监管结果"| KAFKA
  DEVICE -.->|"设备事件 / 控制回执"| KAFKA
  KAFKA -.->|"已裁决的设备命令"| DEVICE
  KAFKA -.->|"状态投影"| TELEMETRY

  classDef client fill:#EDF4FF,stroke:#4D7CBE,color:#153B66,stroke-width:1.4px;
  classDef access fill:#EEF9F5,stroke:#2D8A62,color:#174B36,stroke-width:1.4px;
  classDef business fill:#F4F0FF,stroke:#7C5AB5,color:#3E2369,stroke-width:1.4px;
  ...
  class CLIENT client;
  class ACCESS,EMQX access;
  class PLATFORM,FINANCE,INTEGRATION,ECOSYSTEM,DEVICE,TELEMETRY business;
  class CHANNEL,REGULATOR,UPSTREAM,IOT_SYNC external;
  class KAFKA event;
```

图中 7 个部署单元为 `api-gateway`、`platform-service`、`finance-service`、`integration-service`、`ecosystem-service`、`device-connectivity-service` 和 `telemetry-service`。设备 MQTT 只进入 `device-connectivity-service`；国内省市监管只进入当前 `ecosystem-service`；虚拟电厂等其他外部生态能力不在这张现行图中。
...
| 微信/支付宝/银行/门禁等外部协议 | `integration-service` | 不直接记账或改变订单状态。 |
| 国内省市监管协议、报送、回执、补推和对账 | `ecosystem-service` | 不写业务主库、不直接 MQTT 控制设备；监管没有设备控制权。 |
...
小程序是客户端，不是独立业务服务。M1-M4 与 P1/P2 通过网关调用同一套业务服务；只有前端工程、体验和发布节奏独立。
...
它是**模块化单体**：同一个部署进程中有七个清晰模块，不是无边界“大服务”。模块之间不能互相调用 Mapper 或直接写表；只能经过模块 Service 或业务事件协作。
...
这是一个统一的**防腐层**，不是一个共享线程池：每类适配器必须独立线程池、连接池、超时和重试预算。国内省市监管不属于本服务，监管目标、映射、任务、回执和对账由 `ecosystem-service` 独立维护。

### 4.6 `ecosystem-service` — 国内省市监管

| 内部模块 | 做什么 |
|---|---|
| `regulatory` | 为“一个省级目标 + 可选省内市级目标”管理目标路由、字段映射版本、报送任务、回执、漏推/补推和对账。 |
| `reliability` | 以独立连接池、并发、重试预算、DLQ 和人工处置保证上报闭环；监管失败不阻塞订单、资金或设备。 |
| `evidence` | 保存监管连接引用、请求/回执证据索引和对账差异；原文对象受控进入 MinIO。 |

监管没有设备控制权，不能直连 MQTT、Kafka、MySQL 或设备。虚拟电厂及其他第三方数据/调控仅在 W12 后获得独立批准时才在本服务增加模块。

### 4.7 `telemetry-service` — 遥测与数据分析
...
当前是一个 MySQL 物理实例 `evco-mysql`，内部按 11 个 schema 分开归属。schema 是代码、迁移和权限边界，不代表物理高可用隔离。
...
| `evco_integration` | `integration-service` | 支付、银行、门禁、开票、通知的外部连接、请求/回调审计与死信；不含监管。 |
| `evco_ecosystem` | `ecosystem-service` | 国内省市监管目标、映射、任务、尝试、回执、补推、对账与死信；不含外部平台调控。 |
...
订单、资金和渠道结果不是单向串行调用：设备互联发布订单上报，交易模块发布冻结/结算/退款请求，财务发布渠道命令，外部适配发布已验签渠道结果，财务才发布最终支付/退款状态。每个 Topic、发布者和消费者组见 [Kafka 事件目录](../../contracts/events/README.md)。

订单、资金和渠道结果由不同服务分别记录；通过业务事件协作，不能用跨服务事务或共享表强行锁在一起。长期未完成的冻结、退款或结算必须告警并进入订单—资金对账，不能假设“消息不重复”就一定正确。
...
- [工程架构与数据分库设计](../../docs/design/充电运营平台工程架构与数据分库设计-v1.md)
- [万台单机架构收敛与高可用演进决策](../../docs/design/万台单机架构收敛与高可用演进决策-v1.md)
- [技术选型与单机演进方案](../../docs/design/充电运营平台技术选型与单机演进方案-v1.md)
- [双模式独立部署与 IOT 协同架构决策](../../docs/design/双模式独立部署与IOT协同架构决策-v1.md)
- [平台技术命名规范](../../docs/standards/平台技术命名规范-v1.md)

## 修订记录

| 时间 | 版本 | 修订原因 | 替代内容 |
| --- | --- | --- | --- |
| 2026-08-14 | v1.2 | 用户批准只调整现役文档，消除分层术语歧义。 | 内部 Kafka 协作统一称为“业务事件”，模块内部协作统一引用 Service；不改变服务边界或事件契约。 |
