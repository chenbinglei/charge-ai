...
/** 认证实现：登录锁定窗口、一次性刷新令牌轮换、IDOR 防护的被动撤销与档案组装。 */
@Service
public class AuthServiceImpl implements AuthService {
     ...
     *
     * <p>步骤：验证码一次性校验 → 定位用户（用户名+可选租户）→ 状态前置校验 → BCrypt 比对 → 成功路径签发。
     *
     * @param request 登录请求。
     * @param ip 来源 IP。
        ...
        if (IamUser.STATUS_LOCKED.equals(user.getStatus())) {
            if (user.getLockedUntil() != null && now.isBefore(user.getLockedUntil())) {
                ...
                throw new BusinessException(
                        PlatformErrorCode.UNAUTHENTICATED, "账号已锁定，请约 " + minutes + " 分钟后重试");
            }
    ...
    private void recordPasswordFailure(IamUser user) {
        ...
        boolean windowValid =
                user.getFailWindowStart() != null
                        && Duration.between(user.getFailWindowStart(), now).toMinutes()
                                < windowMinutes;
        ...
        LambdaUpdateWrapper<IamUser> wrapper =
                new LambdaUpdateWrapper<IamUser>()
                        ...
                        .set(
                                IamUser::getFailWindowStart,
                                windowValid ? user.getFailWindowStart() : now)
                        .set(IamUser::getUpdatedAt, now);
        if (lock) {
     ...
     *
     * <p>步骤：按摘要定位令牌（不存在静默成功）→ 撤销会话 → 驱逐权限缓存 → 审计。
     *
     ...
     *
     * <p>步骤：令牌状态与有效期校验 → 会话与用户可用性校验 → 旧令牌置 used → 签发新令牌对 → 会话滑动续期。
     *
     * @param refreshToken 刷新令牌原文。
     * @return 新令牌对登录响应。
     */
    ...
    public LoginVO refresh(String refreshToken) {
        ...
        if (RefreshToken.STATUS_USED.equals(token.getStatus())
        ...
        // 3. 一次性消费旧令牌（CAS active→used），再签发新令牌对。
     ...
     *
     * <p>步骤：会话存在性校验 → 非超管做租户归属校验（IDOR 防护）→ 撤销会话 → 驱逐权限缓存 → 审计。
     *
    ...
    public void revoke(Long sessionId, String reason, AuthContext operator) {
        ...
        // 2. 跨租户防护（W2-D-14 IDOR）：非平台超管只能撤销本租户用户的会话；
    ...
    private RefreshToken loadByHash(String refreshToken) {
        return refreshTokenMapper.selectOne(
                new LambdaQueryWrapper<RefreshToken>()
                        .eq(RefreshToken::getTokenHash, HashUtils.sha256Hex(refreshToken)));
    }
}
