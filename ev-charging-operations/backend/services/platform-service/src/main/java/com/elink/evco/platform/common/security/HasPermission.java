...
/**
 * 接口级两档权限声明（设计包 §7）：GET 接口校验 resource:read，写接口校验 resource:write。
 *
 * <p>权限不足返回 FORBIDDEN(403)；PLATFORM_SUPER 用户短路放行。
 * iotManaged=true 的写接口在 IOT 协同模式下返回 DEPLOYMENT_MODE_READONLY(403)。
 */
