 ...
 *
 * <p>稳定输出规则：参数校验失败归并为 VALIDATION_ERROR；业务异常透传登记码； 未预期异常归并 INTERNAL_ERROR 且不携带堆栈到响应；错误详情只进受控日志。
 */
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleMissingParam(
        ...
        return failure(PlatformErrorCode.VALIDATION_ERROR, "缺少必填参数：" + ex.getParameterName());
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleTypeMismatch(
        ...
        return failure(PlatformErrorCode.VALIDATION_ERROR, "参数类型不匹配：" + ex.getName());
    }
