...
- 实线表示同步连接或协议会话；虚线只表示持久化后的 Kafka 业务事件。数据库所有权由后续表格和 Schema 章节说明，不在总图中重复连线。
...
│       ├── starter/              # evco-starter：web 通用能力（追踪/全局异常/MyBatis 拦截器 + Bearer 鉴权/两档权限，开关控制）
...
W0 不冻结 Java 空包分层。W1 起固定采用**业务域包直挂的传统分层**：业务域包直接挂在服务根包下（如 `platform/iam/`），不设 `module/` 中间层；服务只含单一业务域时不设域包，各层直挂根包（服务名即域）。根包仅保留启动类与 `config`；web 通用能力（原 `common`/`support` 职责）上移共享库 `starter`，服务内不再建 `common`/`support` 包。域包内组织 `controller`、`dto`、`vo`、`entity`、`mapper`、`service`/`service/impl`、`cache`、`util`；`convert`、`client`、`listener`、`job`、`exception` 按需创建。资金、订单、设备控制和监管任务在规模化后在对应 `service` 下追加 `statemachine`、`policy`、`calculator`、`validator`，但不另建全量领域分层。前端和测试也按同一业务域归属组织。不得将域页面、SQL 或第三方 DTO 放入 `shared-kernel` 或 `starter`。职责、依赖方向和中文注释要求见《[全栈研发工程规范](../standards/全栈研发工程规范-v1.md)》。
...
W1 仍须补齐 pnpm workspace、GitHub Actions、Docker Compose、Flyway 迁移、API 契约校验和《全栈研发工程规范》规定的传统分层/中文注释/模块化测试门禁；这些工程资产按 W1 出口统一验收。不得把归档旧路径恢复为新代码入口。
...
| W1：工程跑道 | 建立 11 schema 迁移入口、契约、Compose、资源限额、GitHub Actions、指标和告警；落实前端/测试模块目录与质量门禁。 | 迁移/契约/运行/传统分层/中文注释门禁可重复执行；不开放无业务能力的空路由或消费者。 |
...
| 时间 | 版本 | 修订原因 | 替代内容 |
| --- | --- | --- | --- |
| 2026-08-14 | v1.7 | 对齐 DEC-20260814-021。 | W1 起固定业务模块优先传统分层；前端、测试同步按模块组织；高风险模块在 Service 内强化，不采用全量领域分层。 |
| 2026-08-14 | v1.8 | 用户批准只调整现役文档，收敛传统分层实施口径。 | DEC-20260814-022；GitHub Actions、前端/测试目录和业务事件术语与全栈规范一致。 |
| 2026-08-20 | v1.9 | 06-文档不一致清单 A-3/B-3：分层口径对齐 W2 代码现状（域包直挂、无 `module/` 中间层），libraries 树登记共享库 `starter`（evco-starter）及两层职责。 | 高风险模块 `statemachine`/`policy`/`calculator`/`validator` 降为规模化后按需追加。 |
