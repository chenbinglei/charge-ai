...
每个 Java 服务固定有根包启动类、`config`、`controller`、`dto`、`vo`、`service/impl`、`mapper`、`entity`、`convert`、`client`、`exception` 与 `src/test`。`platform-service` 的各层下再按模块分包，例如 `controller/trade`、`service/trade`、`mapper/trade`；不得将模块页面、SQL 或第三方 DTO 放入 `shared-kernel`。完整职责、依赖方向、命名和中文注释要求以《[全栈研发工程规范](../standards/全栈研发工程规范-v1.md)》为准。
...
contracts/mqtt/                                  # 仅 W12 后外部连接器协议；不包含设备 MQTT 直控
```
...
当前 Compose 默认启动 Nginx、7 个 Java 单元、MySQL、Redis、Kafka、ClickHouse、EMQX、MinIO、Prometheus、Grafana、Alertmanager；MinIO 使用独立本地数据卷承载业务对象和监管证据。Loki、Tempo、OpenSearch、Nacos、Service Mesh、XXL-JOB、Schema Registry 不作为一期常驻组件。
...
| 阶段 | 工作范围 | 完成定义 |
|---|---|---|
| 0：架构跑道 | 建立 7 服务的非空架构跑道、11 schema 迁移入口、契约、Compose、资源限额、CI、指标和告警。 | 每个服务具备健康检查、镜像、配置、最小迁移/契约检查；不开放无业务能力的空路由或消费者。 |
