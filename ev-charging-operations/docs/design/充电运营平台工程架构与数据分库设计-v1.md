...
W0 不冻结 Java 空包分层。W1 起固定采用**业务模块优先的传统分层**：根包仅保留启动类、`config`、`common`、`support`，每个业务模块在 `module/<module>/` 内组织 `controller`、`dto`、`vo`、`service/impl`、`mapper`、`entity`、`convert`、`client`、`listener`、`job` 与 `exception`。资金、订单、设备控制和监管任务在对应 `service` 下追加 `statemachine`、`policy`、`calculator`、`validator`，但不另建全量领域分层。前端和测试也按同一业务模块归属组织。不得将模块页面、SQL 或第三方 DTO 放入 `shared-kernel`。职责、依赖方向和中文注释要求见《[全栈研发工程规范](../standards/全栈研发工程规范-v1.md)》。
