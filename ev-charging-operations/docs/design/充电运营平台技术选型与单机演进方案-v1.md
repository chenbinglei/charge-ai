...
> 架构口径：服务边界、目标工程目录和数据所有权以《万台单机架构收敛与高可用演进决策 v1.1》及《充电运营平台工程架构与数据分库设计 v1.5》为准。当前采用 7 个 Java 部署单元，不是 14 个进程级微服务；早期十万桩/256 GB 和全量微服务表述已废止。
...
当前硬件基线是 Linux x86_64、8 逻辑核、32 GB 内存、独立系统盘，以及 MySQL、ClickHouse、Kafka、MinIO 的独立 NVMe 数据卷；另须具备可访问的离机 S3 兼容对象存储或 NAS 作为备份目标。资源上限和运行组件按《万台单机架构收敛与高可用演进决策 v1.1》的 7 服务预算执行；业务、Kafka、MySQL、ClickHouse、EMQX 与 MinIO 必须配置 cgroup v2 内存与 I/O 限额、CPU 权重；仅低优先级分析/批处理设置硬 CPU 限额，避免 ClickHouse 合并、Redis AOF rewrite、EMQX 会话膨胀、MinIO 大文件写入或 Kafka 积压拖垮订单服务。
...
| 集成/契约测试 | Testcontainers、WireMock、OpenAPI lint、Pact 或 Spring Cloud Contract | 现行覆盖 MySQL/Kafka/Redis/ClickHouse、IOT 管理同步、MQTT 数据/控制协议、支付回调、监管补推和幂等重放。Topic 仅表达协议大版本，`schema_version` 管理该大版本内兼容演进；两者不匹配直接拒绝/DLQ。 |
...
1. 阶段 0 建立 7 个服务的非空架构跑道与 pnpm monorepo、契约目录、Docker Compose、资源限额、环境模板、权限初始化和 `deployment_mode` 服务端拦截；目录与边界见《工程架构与数据分库设计 v1.5》。
