...
> 版本：v1.3（已确认决策；对齐待验证单机起步形态）
> 状态：已确认的双模式 ADR；服务、事件和实时推送实施细节以《[架构基线与服务边界](充电运营平台架构基线与服务边界-v2.md)》和《[Kafka 事件目录](../../contracts/events/README.md)》为准。
> 依据：用户确认的部署、权限、设备接入、遥测、离线订单和数据保留规则。
> 关联：[万台单机架构收敛与高可用演进决策](万台单机架构收敛与高可用演进决策-v1.md)、[总体技术架构与实施蓝图（归档评审历史）](../../archive/design/review-history/充电运营平台总体技术架构与实施蓝图-v1.md)、[菜单架构 v4.1](../requirements/充电运营平台菜单架构-v4.1.md)、[业务规则基线 v4.1](../requirements/充电运营平台业务规则基线-v4.1.md)。
...
| ADR-DUAL-11 | 当前私有化交付采用 8 核 32 GB 普通 Linux 虚拟机/物理机的单机运行包作为**待验证起步形态**；当前采用网关、平台、资金、设备互联、第三方适配、遥测、监管生态共 7 个 Java 部署单元。完成真实设备、目标负载和恢复演练前，不讨论或承诺“万台”、RPO/RTO。Kubernetes/多节点仅作为未来演进预留，不是一期运行时或验收要求。两种形态未来复用同一镜像、配置模型、数据库迁移与运维标准。 |
...
前端只按权限目录决定入口显示；网关和每个业务服务均执行上述校验。付款、退款、余额调整、价格发布、监管补推、设备控制、策略发布和 V2G 提现还需二次确认、职责分离或审批策略。
...
flowchart LR
  ...
  V --> N[统一报文标准化]
  N --> EVT[Kafka\n设备事件]
  EVT --> T[telemetry-service\n状态裁决、分钟归并]
  T --> R[Redis\n最后有效实时值]
  T --> TS[ClickHouse\n永久分钟记录]
  EVT --> OPS[platform-service / ecosystem-service\n订单、告警、监管报送；渠道执行由 integration-service]
```

设备数据与控制协议只由 MQTT 承载，HTTP/HTTPS API 不得接收设备遥测、订单上报或控制回执。MQTT 以 TLS、上游平台凭据/证书、Topic ACL 和 QoS 1 接入；其统一报文至少包括：报文 ID、报文类型、协议版本、来源、固定设备编号/枪口标识、事件时间、接收时间、数据点、值/单位、质量标记、序列号、签名/认证信息和追踪 ID。平台只允许资产库中已存在的设备编号接入，未知编号直接拒绝。命令还包括命令 ID、幂等键、操作者、授权快照、有效期、目标、请求原文、回执原文和最终状态；平台根据自身配置的设备资源标识确定下行目标。MQTT retained message 不得用于启停等控制命令。HTTP/HTTPS API 仅在 IOT 协同模式承载 IOT 的管理、权限和资产同步；HTTP 必须使用 HMAC、时间戳、nonce、幂等键和网络白名单，公网必须使用 HTTPS。设备互联服务只发布已校验事件；Redis 与 ClickHouse 的实时/分钟投影只由 `telemetry-service` 消费事件后写入。
