...
| --- | --- | --- | --- |
| `api-gateway` | 路由、令牌预校验、限流、TraceId、WebSocket 路由 | 不拥有业务事实 | 对象级授权、订单编排、资金决策、MQTT。 |
...
5. 消费者先在本地事务写 Inbox 与自己的业务事实，再提交 offset。连续业务失败才进入生产业务模块的 DLQ；Outbox 发布失败持续重试和告警，不能把未发布事件当作 DLQ 成功处理。
