...
> 版本：v1.1  
> 状态：当前技术架构的执行性决策；服务、目录和技术选型与本文不一致时，以本文为准。  
> 适用基线：10,000 台以内设备；单台 Linux 服务器 8 核 CPU / 32 GB RAM；Docker Compose；设备实际 MQTT 上报周期可变；分钟级最后有效遥测永久保存。  
...
当前采用 **6 个 Java 部署单元 + 模块化领域边界 + 独立逻辑 schema**：
...
5. `integration-service`：支付、银行、监管、门禁/道闸、开票等所有外部适配器；
6. `telemetry-service`：Kafka 消费、实时状态、分钟归并、ClickHouse 写入和受控分析查询。
...
| `integration-service` | 微信/支付宝/银联/银盛、对公转账、监管、门禁/道闸、开票等适配器；外部回调、重试、对账和协议审计 | 所有第三方协议故障、慢调用、证书和升级集中在一个防腐层；独立进程、线程池、连接池和发布节奏，不能拖慢交易或资金。 | 资金账本写入、订单状态机、设备 MQTT、页面权限决策。 |
...
当前一个 MySQL 物理实例 `evco-mysql`，但保留 10 个 schema：`evco_iam`、`evco_master`、`evco_customer`、`evco_trade`、`evco_finance`、`evco_marketing`、`evco_operations`、`evco_integration`、`evco_device_connectivity`、`evco_v2g`。所有技术名称遵循《平台技术命名规范 v1》。
...
- `integration-service` 只拥有 `evco_integration`，统一隔离支付、银行、监管、门禁/道闸、开票等外部协议的凭据引用、请求/回调原文、重试、死信和映射；资金事实仍只由 `finance-service` 写入，运营/监管业务事实仍由其领域服务写入。
...
| 合计容器硬上限 | 23.6 GB | 约 7.2 核 | 内存保留约 8.4 GB 给系统/页缓存/突发；CPU 是**权重而非所有关键链路的硬上限**，只对低优先级分析和批处理设硬限额，生产配额必须由压测校正。 |
...
| `integration-service` 的重试、连接或外部调用占满本服务配额 | 按适配器类别拆出支付/资金通道或监管服务；不得重新并回业务服务。 |
...
| 监管协议/合规团队独立、任务量或故障重试显著增长 | 拆 `regulatory-service`。 |
...
当前固定目录为 6 个可部署 Java 服务。阶段 0 只建立**具备健康检查、镜像、配置、最小迁移入口和契约检查的架构跑道**，不是部署没有职责的空服务；阶段 1 前不开放没有业务能力的路由或消费者。未来候选服务只预留在 ADR/契约中，不创建空 Maven 服务目录。每个阶段按“菜单/场景 → OpenAPI、MQTT Schema、事件、Flyway → 后端与契约测试 → 前端并行 → E2E/性能/恢复验收”纵向交付。

后端并非“整个阶段所有接口完成后再做前端”：接口契约冻结后，前端可用 Mock/生成客户端并行开发；每个菜单阶段的后端、前端、权限、迁移和 E2E 同时验收。
