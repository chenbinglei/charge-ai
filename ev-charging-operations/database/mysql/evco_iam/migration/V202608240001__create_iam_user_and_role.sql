...
-- 时间基线（DEC-20260820-016）：DATETIME 秒级精度、统一北京时间（Asia/Shanghai）
...
--           password_hash、user_type、status、tenant_id
--           （IOT 协同模式直接沿用 IOT 用户 ID 作主键，不再设 iot_user_id 冗余列）
-- ============================================================
...
CREATE TABLE IF NOT EXISTS `iam_user` (
    `id`                   BIGINT       NOT NULL COMMENT '用户 ID（独立模式平台雪花 ID；IOT 协同模式沿用 IOT 推送 ID 作主键，SSO 免登录按本主键定位）',
    ...
    `locked_until`         DATETIME     NULL     COMMENT '锁定截止时间（登录失败 5 次锁 30 分钟）；平台扩展字段，IOT 推送不覆盖',
    `fail_window_start`    DATETIME     NULL     COMMENT '连续失败窗口起点（用于判定“连续 5 次”）；平台扩展字段',
    `fail_count`           INT          NOT NULL DEFAULT 0 COMMENT '连续登录失败次数；成功登录后清零；平台扩展字段',
    `expire_at`            DATETIME     NULL     COMMENT '账号到期日（NULL 表示永不过期，页面清空即存 NULL）；登录时校验，到期自动锁定；平台扩展字段',
    `last_login_at`        DATETIME     NULL     COMMENT '最近登录时间（由 auth_session 登录成功后回写）；平台扩展字段',
    `last_login_ip`        VARCHAR(15)  NULL     COMMENT '最近登录 IP（IPv4；由 auth_session 回写，非 IPv4 来源存 NULL）；平台扩展字段',
    ...
    `deleted_at`           DATETIME     NULL     COMMENT '逻辑删除标记；删除后不可见但审计可追溯，禁止物理删除',
...
CREATE TABLE IF NOT EXISTS `iam_role` (
    ...
    `created_at`  DATETIME     NOT NULL COMMENT '创建时间（北京时间）',
    `updated_at`  DATETIME     NOT NULL COMMENT '更新时间（北京时间）',
    `deleted_at`  DATETIME     NULL     COMMENT '逻辑删除标记；删除后编码可复用',
...
--        WHERE TABLE_SCHEMA = DATABASE() AND CONSTRAINT_TYPE = 'UNIQUE' AND TABLE_NAME = 'iam_user'; => 1
