...
CREATE TABLE IF NOT EXISTS `auth_session` (
    ...
    `ip`          VARCHAR(45)  NULL     COMMENT '登录来源 IP（IPv4/IPv6）；审计用',
    `user_agent`  VARCHAR(255) NULL     COMMENT '登录端 User-Agent 摘要；多端会话管理展示用',
    `expires_at`  DATETIME(3)  NOT NULL COMMENT '会话过期时间（TTL：access_token 有效期对齐，15-30 分钟滑动续期）',
...
CREATE TABLE IF NOT EXISTS `refresh_token` (
    ...
    `expires_at`  DATETIME(3)  NOT NULL COMMENT '过期时间（长 TTL：7-30 天）',
    `created_at`  DATETIME(3)  NOT NULL COMMENT '创建时间（UTC）',
    `updated_at`  DATETIME(3)  NOT NULL COMMENT '更新时间（UTC）',
    PRIMARY KEY (`id`),
